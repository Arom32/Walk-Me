...
class Qwen2LM(TransformerLM):
    ...
    def prepare_lm_input_target(self, sos_emb, text_token, text_token_emb, text_token_len, task_id_emb, speech_token, speech_token_emb, speech_token_len, instruct_token=None, instruct_token_emb=None, instruct_token_len=None):
        ...
        for i in range(len(text_token)):
            # bistream sequence
            # forced off: our inference path (inference(), stream=False) only ever builds unistream
            # lm_input, so training half the batches in bistream format was splitting LoRA's limited
            # capacity across a task we never use at inference time.
            if False and random.random() < 0.5 and speech_token_len[i] / text_token_len[i] > self.mix_ratio[1] / self.mix_ratio[0]:
