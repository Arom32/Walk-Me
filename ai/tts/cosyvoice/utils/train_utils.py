...
def _adam_cls(use_8bit, is_adamw):
    if use_8bit:
        import bitsandbytes as bnb
        return bnb.optim.AdamW8bit if is_adamw else bnb.optim.Adam8bit
    return optim.AdamW if is_adamw else optim.Adam
...
def init_optimizer_and_scheduler(args, configs, model, gan):
    ...
    if gan is False:
        if configs['train_conf']['optim'] == 'adam':
            optimizer = _adam_cls(adam8bit, False)(model.parameters(), **configs['train_conf']['optim_conf'])
        elif configs['train_conf']['optim'] == 'adamw':
            optimizer = _adam_cls(adam8bit, True)(model.parameters(), **configs['train_conf']['optim_conf'])
        else:
        ...
        if configs['train_conf']['optim'] == 'adam':
            optimizer = _adam_cls(adam8bit, False)(model.module.generator.parameters(), **configs['train_conf']['optim_conf'])
        elif configs['train_conf']['optim'] == 'adamw':
            optimizer = _adam_cls(adam8bit, True)(model.module.generator.parameters(), **configs['train_conf']['optim_conf'])
        else:
            raise ValueError("unknown optimizer: " + configs['train_conf'])
        ...
        if configs['train_conf']['optim_d'] == 'adam':
            optimizer_d = _adam_cls(adam8bit, False)(model.module.discriminator.parameters(), **configs['train_conf']['optim_conf_d'])
        elif configs['train_conf']['optim_d'] == 'adamw':
            optimizer_d = _adam_cls(adam8bit, True)(model.module.discriminator.parameters(), **configs['train_conf']['optim_conf_d'])
        else:
            raise ValueError("unknown optimizer: " + configs['train_conf'])
...
def save_model(model, model_name, info_dict):
    ...
    if info_dict["train_engine"] == "torch_ddp":
        if rank == 0:
            ...
            if PeftModel is not None and isinstance(raw_model, PeftModel):
                state = get_peft_model_state_dict(raw_model)  # adapter weights only, keeps checkpoints small
            else:
